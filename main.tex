...
Le projet SWUMANOID vise à développer le firmware d'un contrôleur de robot humanoïde.
Le système embarqué est construit sur le système de compilation \textbf{PlatformIO}, utilisant le
framework \textbf{Arduino} sur un microcontrôleur \textbf{Espressif ESP32-S3}.
Le firmware gère le contrôle en temps réel des servomoteurs via un bus RS-485 Dynamixel, la mesure
inertielle, la communication sans fil via un point d'accès Wi-Fi, et la lecture de mouvements à
partir de fichiers stockés sur le système de fichiers flash interne.

L'architecture logicielle est organisée autour d'une machine à états finis
(\texttt{AppState} : \texttt{Boot}, \texttt{Idle}, \texttt{Loading}, \texttt{Ready},
\texttt{Running}, \texttt{Stopping}, \texttt{Error}), pilotée par des commandes UDP reçues
depuis une interface web. Les données des capteurs, la télémétrie des moteurs et l'état
du système sont périodiquement diffusés en retour au client.

\subsection{Mise à niveau matérielle : AtomS3 \texorpdfstring{$\rightarrow$}{->} AtomS3R}

Le firmware d'origine a été écrit et validé pour la carte de développement \textbf{M5Stack AtomS3}.
À mon arrivée au laboratoire, le matériel avait été mis à niveau vers le \textbf{M5Stack AtomS3R}.
Bien que les deux cartes partagent le même SoC ESP32-S3 et un facteur de forme similaire, elles
diffèrent par deux composants matériels qui affectent directement le firmware :
...
Ma première mission technique consistait à \textbf{migrer la couche des pilotes matériels}
d'un accès aux registres codé à la main et spécifique à la carte, vers la bibliothèque
d'abstraction \textbf{M5Unified}, qui prend en charge nativement à la fois l'AtomS3 et l'AtomS3R.
La migration a été circonscrite à deux pilotes, en préservant entièrement le reste de
l'architecture du firmware :
...
La contrainte était explicite : \textit{ne pas modifier la logique globale de la machine à états}.
Seule la couche matérielle devait être adaptée.
...
\subsection{Migration du Code (De l'AtomS3 vers l'AtomS3R)}
...
\subsection{Mesure du RSSI et Dashboard}
...
\subsection{Validation du Bus RS-485}
...
Une fois ce montage en place, il restera à reprogrammer le baudrate dans l'EEPROM de chaque moteur pour qu'il corresponde à celui du microcontrôleur, puis à valider la stabilité du bus (taux de perte de paquets, erreurs CRC) au nouveau débit. Le passage à 1\,Mbps suffit déjà théoriquement à atteindre les 200\,Hz visés avec une marge confortable, les 3\,Mbps constituant une réserve pour la montée en charge vers les 21 moteurs du SWUMANOID.


\section{Nouvelle Architecture Web}
...
\section{Problèmes Rencontrés et Solutions Implémentées}
Le passage des tests virtuels au robot physique SWUMANOID a montré des problèmes qui ont nécessité des refontes algorithmiques et logiques.

\subsection{Gestion Critique du Couple et Moteurs Inactifs}

\paragraph{Problème identifié :} Lors de l'envoi du robot vers sa position initiale, seulement quelques moteurs réagissaient. Pire encore, les moteurs des membres inférieurs se désactivaient totalement, laissant le robot s'affaisser dangereusement.\\

Le problème était que sur les 21 moteurs du robot, seulement 18 sont utilisés dans le mouvement, les moteurs 1, 18 et 19 sont sensés restés fixes durant le mouvement. Ainsi ces derniers recevaient une consigne de position fixée à \texttt{0}. Or la valeur \texttt{0} correspond à la butée mécanique extrême, ce qui déclenchait la sécurité interne de surcharge (\textit{Overload Error}) et coupait le couple.

\paragraph{Résolution :}  J'ai corrigé ce problème en sécurisant le control du robot, désormais dès que des moteurs sont détectés sur le bus, le couple est désactivé, rendant le robot souple et permettant la manipulation et ce même lorsque le fichier de mouvement est uploadé. Le couple s'active qu'à partir du moment où l'on envoie le robot à sa position initiale, de plus tous les moteurs détectés mais inactifs dans le fichier de mouvement uploadé sont envoyés à la position centrale de sécurité par défaut (\texttt{2048 ticks}) avec le couple activé.\\

De plus, pour pallier les limitations du convertisseur de fichiers \texttt{.dat} (qui ignorait certains IDs moteurs), j'ai implémenté la prise en charge directe des fichiers pré-convertis \texttt{.inc} contenant la matrice complète des positions des 21 moteurs. Ces fichiers, générés par le pipeline de simulation de Nakashima-sensei, contiennent des matrices C déjà formatées.

\subsection{Cinématique : Saccades et Fluidité du Mouvement}

\paragraph{Problème identifié.} L'interpolation linéaire initialement utilisée pour relier les frames de mouvement garantissait la continuité de la position, mais pas celle de ses dérivées : à chaque nœud, la vitesse commandée changeait de façon discontinue (accélération théoriquement infinie), ce qui se traduisait par des mouvements saccadés. Le déplacement vers la position initiale était également trop brusque, et un bug logiciel figeait la fréquence de contrôle à 15~Hz indépendamment des consignes de l'interface. Enfin, lors de la lecture cyclique d'un mouvement (répétition du même fichier), la dernière frame ne coïncidait généralement pas avec la première : le rebouclage provoquait un saut de position et de vitesse, donc un à-coup mécanique à chaque début de cycle.

\paragraph{Fréquence dynamique et rééchantillonnage.} Afin d'offrir une flexibilité totale sur le contrôle temporel, la logique de gestion des paramètres a été inversée. Désormais, la fréquence de contrôle ($f_{\mathrm{Hz}}$), la durée d'un cycle ($T_{\mathrm{cycle}}$) et le nombre de répétitions ($N_{\mathrm{plays}}$) sont les seules variables définies par l'utilisateur. Le nombre de frames par cycle ($F$) n'est plus une contrainte imposée par le fichier source, mais une variable calculée dynamiquement :
...
\paragraph{Résolution du système cyclique : décomposition de Sherman--Morrison et algorithme de Thomas.} Le système~\eqref{eq:matrice-cyclique} n'est pas tridiagonal à cause des deux coefficients de coin $A_{0,N-1} = A_{N-1,0} = 1$. On l'écrit comme une perturbation de rang~1 d'une matrice tridiagonale ordinaire $T$ :
\begin{equation}
    A = T + u\,v^{\mathsf{T}}, \qquad
...
\textbf{Rampe Minimum-Jerk :} Pour le déplacement vers la position initiale, le firmware C++ génère désormais une trajectoire d'approche polynomiale de degré 5 (\textit{minimum-jerk}). L'équation temporelle normalisée est :
\begin{equation}
...
\subsection{Fiabilité de la Position Initiale (Boucle Fermée)}
\paragraph{Problème identifié :} L'interface web autorisait le lancement du mouvement (\textit{Play}) de manière "aveugle", dès que l'utilisateur cliquait sur le bouton d'initialisation, sans garantie que les moteurs aient physiquement atteint leur position (notamment à cause de paquets réseau perdus).

\paragraph{Résolution :} J'ai transformé cette logique en une \textbf{architecture en boucle fermée}. Lors de la commande d'initialisation, le microcontrôleur lit en continu la position réelle des moteurs via un \textit{Sync Read}. Il ne renvoie l'état de confirmation (\texttt{initial\_pose}) via SSE que lorsque tous les moteurs sont dans une tolérance stricte de $\pm 30$ ticks (environ $\pm 2,6^\circ$). Côté web, le bouton \textit{Play} reste strictement verrouillé tant que cette confirmation matérielle n'est pas reçue par le JavaScript.\\

Le problème est que cette architecture a sur-contraint l'utilisation du bouton \textit{Play}. Parfois, même lorsque les moteurs sont physiquement à leur position initiale, un faux négatif logiciel empêche le lancement du mouvement. Pour pallier cette rigidité, et grâce à Daito-san, un système de \textit{bypass} des contraintes a été mis en place sous la forme d'un interrupteur de forçage manuel (\textit{Safety Override}).\\

Ce système permet de désactiver individuellement trois conditions de sécurité préalables au lancement :
\begin{enumerate}
    \item \textbf{\textit{Ignore start-frame match}} : désactive la vérification stricte validant que la position physique actuelle correspond exactement à la position théorique de la frame de départ ;
    \item \textbf{\textit{Ignore state check}} : ignore la validation de l'état global du système (télémétrie nominale, détection de tous les moteurs, absence d'erreurs de courant/tension) ;
    \item \textbf{\textit{Ignore recalc pending}} : autorise le lancement même si les paramètres de temps ou de fréquence (\textit{Motion Settings}) ont été modifiés sans qu'un recalcul de la trajectoire n'ait été effectué.
\end{enumerate}

En pratique, lors de nos essais sur le robot physique, ce sont très majoritairement les deux premières conditions qui n'étaient pas respectées. De légères fluctuations des capteurs ou la perte de quelques paquets RS-485 lors du scan suffisaient à générer un faux positif et à bloquer intempestivement le système. En cochant ces options via le \textit{Safety Override}, l'opérateur assume l'état matériel et le bouton \textit{Play} est forcé à l'état actif. Cela redonne le contrôle total à l'utilisateur et évite que la sur-sécurisation du logiciel ne devienne un frein à l'expérimentation matérielle.

\subsection{Implémentation de la Télémétrie : Résilience, Latence et Gestion Mémoire}
...
\texttt{cmd, pos, cur} & \texttt{int16, int32, int16} & (Par moteur) Consigne, Position, Courant \\ \hline
\texttt{volt} & \texttt{uint16} & (Par moteur) Tension \\ \hline
\end{tabular}
...
\section{Calcul des angles du SWUMANOID par l'IMU}
...
\subsection{Changement de repère : alignement matériel (Z-Up)}
\label{subsec:calib}
...
\subsection{Opération de tare (Set Zero)}
\label{subsec:tare}
